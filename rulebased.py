...
def generate_sentence(input_words):
    ...
    for token in doc:
        print(f"Word: {token.text}, POS: {token.pos_}, Dependency: {token.dep_}, Entity: {token.ent_type_}, Head: {token.head.text}")
    ...
    # Ensure the subject is present, otherwise provide a fallback
    if not subject:
        subject = "Someone"
    
    # Handle case where there's no verb, we provide a default fallback verb
    if not verb and obj:
        verb = "want"  # Default verb if missing
    ...
    # Rule 4: Handle named entities
    if named_entities:
        sentence += f" Maybe have some {', '.join(named_entities)}?"

    return sentence

# Example 1: Input words
input_words1 = ["I", "hungry", "eat", "Oreo"]
